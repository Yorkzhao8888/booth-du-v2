...
export default function YieldTracking() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [statsRes] = await Promise.all([
        api.get<any>('/edxx/fab/yield/stats?days=7'),
      ]);
      if (statsRes?.success) {
        setStats(statsRes);
      }
      ...
      const recordsRes = await api.get<any>('/edxx/fab/yield/all');
  ...
  const fetchWorkOrders = async () => {
    try {
      const res = await api.get<any>('/edxx/fab/queue?status=in_progress');
  ...
  const handleAdd = async () => {
    try {
      ...
      const res = await api.post<any>('/edxx/fab/yield/record', values);
      if (res) { // api.ts 解包后 res 即业务数据
