...
const StocktakeExec: React.FC = () => {
  ...
  const fetchStocktakes = async () => {
    setLoading(true);
    try {
      const res = await api.get('/edxx/wh/stocktakes');
  ...
  const fetchInventory = async () => {
    try {
      const res = await api.get('/edxx/wh/inventory');
  ...
  const handleCreate = async (values: any) => {
    try {
      ...
      await api.post('/edxx/wh/stocktakes', { items, remark: values.remark });
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      await api.post(`/edxx/wh/stocktakes/${currentSo.id}/submit`, { items });
