...
const FabOperations = () => {
  ...
  const submitAndon = async () => {
    try {
      ...
      const res = await api.post<any>('/edxx/fab/andon', {
        type: v.type,
        severity: v.severity,
        message: v.message,
  ...
  const fetchActiveOrders = async () => {
    setLoading(true);
    try {
      const res = await api.get<any>('/edx/work-orders?status=in_progress');
  ...
  const fetchOperations = async (workOrderId: number) => {
    try {
      const res = await api.get<any>(`/edxx/fab/operations?workOrderId=${workOrderId}`);
  ...
  const handleReport = async (values: any) => {
    try {
      await api.post('/edxx/fab/report', { workOrderId: selectedWo.id, ...values });
  ...
  const handleComplete = async () => {
    try {
      await api.post('/edxx/fab/complete', { workOrderId: selectedWo.id });
  ...
  const handleAdvanceStage = async (targetStage: string) => {
    try {
      const res = await api.post<any>('/edxx/fab/stage/advance', { 
