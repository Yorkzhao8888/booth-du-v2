...
const DlExec: React.FC = () => {
  ...
  const fetchTasks = async () => {
    setLoading(true);
    try {
      const res = await api.get('/edxx/dl/tasks');
      setTasks(res.items || []);
    } catch (e) { /* ignore */ }
    setLoading(false);
  };
  ...
  const handleAction = async (values: any) => {
    ...
    try {
      if (actionType === 'start') {
        await api.post(`/edxx/dl/tasks/${currentTask.id}/start`);
        message.success('开始配送');
      } else if (actionType === 'complete') {
        await api.post(`/edxx/dl/tasks/${currentTask.id}/complete`, { signedBy: values.signedBy });
        message.success('配送完成');
      } else {
        await api.post(`/edxx/dl/tasks/${currentTask.id}/exception`, { reason: values.reason });
