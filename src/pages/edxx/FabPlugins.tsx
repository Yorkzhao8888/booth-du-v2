...
export default function FabPlugins() {
  ...
  const fetchCatalog = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get(`/edxx/fab/plugins/catalog${params.toString() ? `?${params}` : ''}`);
      setItems(res?.items || []);
    } catch (e: any) {
  ...
  useEffect(() => {
    fetchCatalog();
    api.get('/edxx/fab/stations').then((res) => {
  ...
  const register = async (values: any) => {
    try {
      ...
      await api.post(`/edxx/fab/station/${values.station_id}/capabilities/register`, {
  ...
  const toggle = async (row: CapItem) => {
    ...
    try {
      if (next === 'unregister') {
        await api.post(`/edxx/fab/station/${row.station_id}/capabilities/${encodeURIComponent(row.capability_code)}/unregister`, {});
        message.success(`能力 ${row.capability_code} 已停用 (不影响既有工单状态机)`);
      } else {
        await api.post(`/edxx/fab/station/${row.station_id}/capabilities/register`, {
  ...
  const runMatch = async () => {
    ...
    try {
      const res = await api.get(`/edxx/fab/orders/${oid}/capability-match`);
