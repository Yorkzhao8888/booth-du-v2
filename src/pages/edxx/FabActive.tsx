...
const FabActive: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await apiGet<{ items: WorkOrderData[]; total: number }>('/edxx/fab/active');
      setOrders(res.items);
    } catch {
      // ignore
    } finally {
  ...
  const handleStart = async (id: number) => {
    setActionId(id);
    try {
      await apiPost(`/edxx/fab/work-orders/${id}/start`);
  ...
  const handleComplete = async (id: number) => {
    setActionId(id);
    try {
      await apiPost(`/edxx/fab/work-orders/${id}/complete`);
