...
export default function FabTelemetry() {
  ...
  // 写操作(登记配置/模拟心跳)走后端 requireHat('FAB'): 仅 FAB 帽(edxx); du/dx/edx/em 只读
  const canWrite = role === 'edxx';
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [h, c, e] = await Promise.all([
        api.get<any>('/edxx/fab/telemetry/health'),
        api.get<any>('/edxx/fab/telemetry/configs'),
        api.get<any>('/edxx/fab/equipment'),
      ]);
      setHealth(h);
  ...
  const saveConfig = async (values: any) => {
    try {
      await api.post('/edxx/fab/telemetry/configs', {
  ...
  const toggleConfig = async (row: TelemetryConfig) => {
    try {
      await api.post(`/edxx/fab/telemetry/configs/${row.id}/toggle`, {});
  ...
  const removeConfig = async (row: TelemetryConfig) => {
    try {
      await api.delete(`/edxx/fab/telemetry/configs/${row.id}`);
  ...
  const demoBeat = async () => {
    setBeatLoading(true);
    try {
      const res = await api.post<any>('/edxx/fab/telemetry/demo/beat', {});
  ...
  const fetchChart = useCallback(async () => {
    ...
    try {
      const [t, o] = await Promise.all([
        api.get<any>(`/edxx/fab/equipment/${chartEqId}/telemetry?demo_source=${chartDemo}`),
        api.get<any>(`/edxx/fab/equipment/${chartEqId}/oee`).catch(() => null),
      ]);
