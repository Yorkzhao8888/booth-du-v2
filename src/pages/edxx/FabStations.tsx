...
export default function FabStations() {
  ...
  const reportStatus = async (station: Station, newState: string) => {
    try {
      await api.post<any>(`/edxx/fab/station/${station.id}/report-status`, {
  ...
  const triggerFault = async (station: Station) => {
    try {
      const res = await api.post<any>(`/edxx/fab/station/${station.id}/fault`, {
              ...
              return (
                <Col xs={24} sm={12} lg={8} xl={6} key={s.id}>
                  <Card
                    hoverable
                    size="small"
                    onClick={() => navigate(`/edxx/fab/station/${s.id}`)}
                    title={
