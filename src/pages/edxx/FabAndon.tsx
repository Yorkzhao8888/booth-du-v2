...
const AndonBoard = () => {
  ...
  const fetchBoard = useCallback(async () => {
    try {
      const res = await api.get('/edxx/fab/andon/board');
      setItems(res.items || []);
      setStats(res.stats || {});
    } catch { /* ignore */ }
  }, []);
  ...
  const doAssign = async (ev: any) => {
    try {
      await api.post(`/edxx/fab/andon/${ev.id}/assign`, {});
  ...
  const doResolve = async () => {
    ...
    try {
      await api.post(`/edxx/fab/andon/${resolveModal.id}/resolve`, { solution: v.solution });
