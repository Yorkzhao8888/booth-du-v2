/**
 * FAB-MES-01 设备台账页（/edxx/fab/equipment）
...
export default function FabEquipment() {
  ...
  const load = async () => {
    ...
    try {
      const [eqRes, stRes] = await Promise.all([
        api.get('/edxx/fab/equipment'),
        api.get('/edxx/fab/stations').catch(() => ({ data: { data: [] } })),
      ]);
  ...
  const onCreate = async (values: any) => {
    try {
      await api.post('/edxx/fab/equipment', {
  ...
  const onStatusChange = async (values: any) => {
    ...
    try {
      await api.post(`/edxx/fab/equipment/${statusTarget.id}/status`, {
            ...
            return (
              <Col xs={24} sm={12} lg={8} key={eq.id}>
                <Card
                  size="small"
                  hoverable
                  onClick={() => nav(`/edxx/fab/equipment/${eq.id}`)}
                  title={
                    <Space size={6}>
