  ...
  const orderColumns = [
    ...
    { 
      ...
      render: (_: unknown, record: OrderTask) => (
        <Space>
          {record.status === 'pending' && <Button size="small" type="link" onClick={() => navigate(`/emxx/fulfillment/${record.id}/pick`)}>拣货</Button>}
          {record.status === 'picking' && <Button size="small" type="link" onClick={() => navigate(`/emxx/fulfillment/${record.id}/pack`)}>打包</Button>}
          {record.status === 'packing' && <Button size="small" type="link" onClick={() => navigate(`/emxx/fulfillment/${record.id}/deliver`)}>交付</Button>}
        </Space>
      )
    },
  ];
      ...
      children: (
        <div>
          ...
          <Space>
            ...
            <Button onClick={() => navigate('/edxx/wh/inbound')}>入库登记</Button>
          </Space>
