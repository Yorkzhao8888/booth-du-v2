...
export default function FabStationDetail() {
  ...
  const isReadOnly = user?.role !== 'exx';
  ...
  const fetchStation = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get<any>(`/exx/fab/stations/${id}`);
  ...
  const fetchCaps = useCallback(async () => {
    if (!id) return;
    try {
      const res = await api.get<any>(`/exx/fab/station/${id}/capabilities`);
  ...
  const submitStatus = async () => {
    ...
    try {
      await api.post<any>(`/exx/fab/station/${id}/report-status`, { state: toApiState(newStatus), reason: 'Manual report from detail page' });
  ...
  const submitFault = async () => {
    ...
    try {
      const res = await api.post<any>(`/exx/fab/station/${id}/fault`, { reason: faultReason, strategy: faultStrategy });
      if (res) {
        // 解包后 res 即 fault 返回体
        const d = res || {};
  ...
  const deployAgent = async () => {
    ...
    try {
      ...
      await api.post<any>(`/exx/fab/station/${id}/deploy-agent`, { agent_id: agentId, note: 'Placeholder registration (LoRA pending)' });
    ...
    return (
      <div style={{ padding: 48 }}>
        <Empty description="Station 不存在">
          <Button onClick={() => navigate('/exx/station')}>返回 Station 列表</Button>
        </Empty>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '20px 24px', background: '#F5F7FA', minHeight: '100%' }}>
      <Space style={{ marginBottom: 16 }}>
        <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/exx/station')}>返回列表</Button>
