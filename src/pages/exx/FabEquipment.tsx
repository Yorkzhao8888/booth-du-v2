/**
 * FAB-MES-01 设备台账页（/exx/fab/equipment）
...
export default function FabEquipment() {
  ...
  const load = async () => {
    ...
    try {
      const [eqRes, stRes] = await Promise.all([
        api.get('/exx/fab/equipment'),
        api.get('/exx/fab/stations').catch(() => ({ data: { data: [] } })),
      ]);
  ...
  const onCreate = async (values: any) => {
    try {
      await api.post('/exx/fab/equipment', {
  ...
  const onStatusChange = async (values: any) => {
    ...
    try {
      await api.post(`/exx/fab/equipment/${statusTarget.id}/status`, {
            ...
            return (
              <Col xs={24} sm={12} lg={8} key={eq.id}>
                <Card
                  size="small"
                  hoverable
                  onClick={() => nav(`/exx/fab/equipment/${eq.id}`)}
                  title={
                    <Space size={6}>
