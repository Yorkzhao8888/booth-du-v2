...
const SvcExec: React.FC = () => {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get(`/exx/svc/tasks${params}`);
      setTasks(res.items || []);
    } catch (e) { /* ignore */ }
    setLoading(false);
  }, [categoryFilter]);
  ...
  const handleAction = async (values: any) => {
    ...
    try {
      if (actionType === 'start') {
        await api.post(`/exx/svc/tasks/${currentTask.id}/start`);
        message.success('开始服务');
      } else if (actionType === 'complete') {
        await api.post(`/exx/svc/tasks/${currentTask.id}/complete`, { result: values.result, remark: values.remark });
        message.success('服务完成');
      } else {
        await api.post(`/exx/svc/tasks/${currentTask.id}/exception`, { reason: values.reason });
