...
export default function FabPlugins() {
  ...
  const fetchCatalog = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get(`/exx/fab/plugins/catalog${params.toString() ? `?${params}` : ''}`);
      setItems(res?.items || []);
    } catch (e: any) {
  ...
  useEffect(() => {
    fetchCatalog();
    api.get('/exx/fab/stations').then((res) => {
  ...
  const register = async (values: any) => {
    try {
      ...
      await api.post(`/exx/fab/station/${values.station_id}/capabilities/register`, {
  ...
  const toggle = async (row: CapItem) => {
    ...
    try {
      if (next === 'unregister') {
        await api.post(`/exx/fab/station/${row.station_id}/capabilities/${encodeURIComponent(row.capability_code)}/unregister`, {});
        message.success(`能力 ${row.capability_code} 已停用 (不影响既有工单状态机)`);
      } else {
        await api.post(`/exx/fab/station/${row.station_id}/capabilities/register`, {
  ...
  const runMatch = async () => {
    ...
    try {
      const res = await api.get(`/exx/fab/orders/${oid}/capability-match`);
