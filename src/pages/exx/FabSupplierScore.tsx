...
export default function ExxFabSupplierScore() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get('/exx/fab/score/dashboard');
      setDash(res || null);
    } catch (e: any) {
  ...
  const saveConfig = async () => {
    try {
      ...
      await api.post('/exx/fab/score/config', { weights, min_samples: v.min_samples, window_days: v.window_days });
  ...
  const refresh = async () => {
    try {
      await api.post('/exx/fab/score/refresh');
  ...
  const searchOut = async () => {
    ...
    if (!id) return;
    try {
      const res = await api.get(`/exx/fab/score/${id}`);
      setOutCard(res || null);
    } catch (e: any) {
