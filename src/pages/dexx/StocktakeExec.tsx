...
const StocktakeExec: React.FC = () => {
  ...
  const fetchStocktakes = async () => {
    setLoading(true);
    try {
      const res = await api.get('/dexx/wh/stocktakes');
  ...
  const fetchInventory = async () => {
    try {
      const res = await api.get('/dexx/wh/inventory');
  ...
  const handleCreate = async (values: any) => {
    try {
      ...
      await api.post('/dexx/wh/stocktakes', { items, remark: values.remark });
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      await api.post(`/dexx/wh/stocktakes/${currentSo.id}/submit`, { items });
