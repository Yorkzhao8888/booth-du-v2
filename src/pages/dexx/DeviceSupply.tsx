...
const DeviceSupply: React.FC = () => {
  ...
  const fetchDevices = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get('/dexx/wh/devices');
  ...
  const handleCreate = async (values: any) => {
    try {
      await api.post('/dexx/wh/devices', values);
  ...
  const handleDispatch = async (values: any) => {
    ...
    try {
      await api.post(`/dexx/wh/devices/${currentDevice.id}/dispatch`, values);
  ...
  const handleMaintenance = async (values: any) => {
    if (!currentDevice) return;
    try {
      await api.post(`/dexx/wh/devices/${currentDevice.id}/maintenance`, values);
  ...
  const handleCompleteMaintenance = async (logId: number) => {
    if (!currentDevice) return;
    try {
      await api.post(`/dexx/wh/devices/${currentDevice.id}/maintenance/complete`, { logId });
  ...
  const fetchMaintenanceLogs = async (deviceId: number) => {
    try {
      const res = await api.get(`/dexx/wh/devices/${deviceId}/maintenance`);
