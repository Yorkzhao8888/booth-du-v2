...
const PlazaSupply: React.FC = () => {
  ...
  const fetchResources = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get('/dexx/wh/plaza-resources');
  ...
  const fetchBookings = useCallback(async () => {
    try {
      const res = await api.get('/dexx/wh/plaza-bookings');
  ...
  const handleCreateResource = async (values: any) => {
    try {
      await api.post('/dexx/wh/plaza-resources', values);
  ...
  const handleBook = async (values: any) => {
    ...
    try {
      await api.post(`/dexx/wh/plaza-resources/${currentResource.id}/book`, {
  ...
  const handleRelease = async (bookingId: number) => {
    try {
      await api.post(`/dexx/wh/plaza-bookings/${bookingId}/release`);
      message.success('已释放');
      fetchResources();
      fetchBookings();
