/**
 * FAB-MES-01 单设备 OEE 明细页（/dexx/fab/equipment/:id）
...
export default function FabEquipmentOee() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [eqRes, oeeRes] = await Promise.all([
        api.get(`/dexx/fab/equipment/${id}`).catch(() => ({ data: { data: null } })),
        api.get(`/dexx/fab/equipment/${id}/oee?${windowParams()}`),
      ]);
  ...
  return (
    <div style={{ padding: 20, maxWidth: 1100, margin: '0 auto' }}>
      <Button type="text" icon={<ArrowLeftOutlined />} onClick={() => nav('/dexx/fab/equipment')} style={{ marginBottom: 8, paddingLeft: 0 }}>
        返回设备台账
      </Button>
