...
export default function FabTelemetry() {
  ...
  // 写操作(登记配置/模拟心跳)走后端 requireHat('FAB'): 仅 FAB 帽(dexx); du/dx/dex/em 只读
  const canWrite = role === 'dexx';
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [h, c, e] = await Promise.all([
        api.get<any>('/dexx/fab/telemetry/health'),
        api.get<any>('/dexx/fab/telemetry/configs'),
        api.get<any>('/dexx/fab/equipment'),
      ]);
      setHealth(h);
  ...
  const saveConfig = async (values: any) => {
    try {
      await api.post('/dexx/fab/telemetry/configs', {
  ...
  const toggleConfig = async (row: TelemetryConfig) => {
    try {
      await api.post(`/dexx/fab/telemetry/configs/${row.id}/toggle`, {});
  ...
  const removeConfig = async (row: TelemetryConfig) => {
    try {
      await api.delete(`/dexx/fab/telemetry/configs/${row.id}`);
  ...
  const demoBeat = async () => {
    setBeatLoading(true);
    try {
      const res = await api.post<any>('/dexx/fab/telemetry/demo/beat', {});
  ...
  const fetchChart = useCallback(async () => {
    ...
    try {
      const [t, o] = await Promise.all([
        api.get<any>(`/dexx/fab/equipment/${chartEqId}/telemetry?demo_source=${chartDemo}`),
        api.get<any>(`/dexx/fab/equipment/${chartEqId}/oee`).catch(() => null),
      ]);
