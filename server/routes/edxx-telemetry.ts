...
const INGEST_META = {
  ...
  ingest_endpoint: 'POST /edxx/fab/telemetry/ingest',
...
// 写操作闸门(与 PK-01 station-capabilities 同款): 非 GET/HEAD 需 du/dx/edx 或 FAB 帽;
  ...
  if (req.method !== 'GET' && req.method !== 'HEAD') {
    ...
    if (user) {
      ...
      if (!isManager && !isFabHat) {
        return res.status(403).json({ success: false, error: 'FORBIDDEN', message: '采集编排仅 EX/EDX 或 FAB 帽持有者可用' });
      }
    }
  }
  next();
...
// ---------------------------------------------------------------------------
// POST /edxx/fab/telemetry/ingest —— 边缘上报(幂等)
...
// ---------------------------------------------------------------------------
// GET /edxx/fab/equipment/:id/telemetry?from=&to=&metric=&demo_source=all|exclude|only
...
// ---------------------------------------------------------------------------
// GET /edxx/fab/telemetry/health —— 采集在线状态
...
// POST /edxx/fab/telemetry/configs —— 登记/更新采集配置(幂等 upsert: org+equipment+metric 唯一; station 自动带出)
...
// POST /edxx/fab/telemetry/configs/:id/toggle —— 启停采集配置(幂等取反)
...
// DELETE /edxx/fab/telemetry/configs/:id —— 删除采集配置(幂等; 遥测历史数据保留)
...
// ---------------------------------------------------------------------------
// POST /edxx/fab/telemetry/demo/beat —— 内置模拟上报通道(仅链路联调)
