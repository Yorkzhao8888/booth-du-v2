...
const INGEST_META = {
  ...
  ingest_endpoint: 'POST /dexx/fab/telemetry/ingest',
...
// ---------------------------------------------------------------------------
// POST /dexx/fab/telemetry/ingest —— 边缘上报(幂等)
...
// ---------------------------------------------------------------------------
// GET /dexx/fab/equipment/:id/telemetry?from=&to=&metric=&demo_source=all|exclude|only
...
// ---------------------------------------------------------------------------
// GET /dexx/fab/telemetry/health —— 采集在线状态
...
// POST /dexx/fab/telemetry/configs —— 登记/更新采集配置(幂等 upsert: org+equipment+metric 唯一; station 自动带出)
...
// POST /dexx/fab/telemetry/configs/:id/toggle —— 启停采集配置(幂等取反)
...
// DELETE /dexx/fab/telemetry/configs/:id —— 删除采集配置(幂等; 遥测历史数据保留)
...
// ---------------------------------------------------------------------------
// POST /dexx/fab/telemetry/demo/beat —— 内置模拟上报通道(仅链路联调)
