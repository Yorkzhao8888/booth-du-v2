/**
 * edxx FAB-MES 路由 (TECH-DEBT-1 从 edxx-modules.ts 拆出)
 * 覆盖: Station-OS 产线/作业站融合(FAB-MES-05) / 产线视角 / 设备台账+OEE(FAB-MES-01) / 保养 / 安灯异常中心(FAB-MES-03)
 * 挂载: /api/booth/edxx (见 edxx-modules.ts 聚合)
...
// 导出供 edxx-fab-trace.ts 复用 (FAB-MES-02)
...
// X 层(ex/edxx)只读时剥离价格字段; du/dx/dm 可看全量 (价格边界红线)
const stripFabReadFor = (user: JwtPayload, data: unknown): unknown => {
  if (user && ['ex', 'edxx'].includes(user.role)) return stripPriceFields(data as any);
  return data;
};

// ====== FAB-MES-05: Station-OS 产线/作业站融合 ======

// 7. GET /edxx/fab/stations: Station 列表
...
// 8. GET /edxx/fab/stations/:id: 单站详情
...
// 1. POST /edxx/fab/station/:id/assign-order: Station 接单
...
// 2. POST /edxx/fab/station/:id/report-status: 站状态上报
...
// 3. POST /edxx/fab/station/:id/deploy-agent: 部署 Agent (占位待 LoRA, 仅登记)
...
// 4. POST /edxx/fab/station/:id/invoke-agent: 调用 Agent (占位待 LoRA, 必须过 access_token 鉴权)
...
// 5. POST /edxx/fab/station/:id/report-agent-status: Agent 状态上报
...
// 6. POST /edxx/fab/station/:id/fault: 故障上报 → 按 fault_strategy 传播
...
// 9. GET /edxx/fab/zone/:stage: 产线视角按阶段查询（前置/制作/包装/分拣）
...
// GET /edxx/fab/equipment 设备台账列表（含当前状态/OEE/上次保养）
...
// POST /edxx/fab/equipment/:id/status 变更设备状态 + 停机原因
...
// GET /edxx/fab/equipment/oee/dashboard 全厂 OEE 汇总（先注册，避免与 :id 冲突）
...
// GET /edxx/fab/equipment/:id 单设备详情（含最近状态流水 + 挂载工位）
...
// GET /edxx/fab/equipment/:id/oee 单设备 OEE
...
// GET /edxx/fab/maintenance/plans 保养计划列表（含 overdue 预警）
...
// POST /edxx/fab/maintenance/plans/:id/done 完成保养
...
// DELETE /edxx/fab/equipment/:id — 删除设备并级联清理状态流水与保养计划
  ...
  try {
    ...
    try {
      ...
      if (e?.code === '23503') {
        ...
        return res.status(409).json({ success: false, error: 'equipment still referenced by fab operations', code: '23503', data: { id, operation_refs: refs.rows[0].c, hint: 'POST /api/booth/edxx/fab/operations/cleanup-by-equipment {"equipment_id":' + id + '} 先清理该设备报工引用' } });
      }
      throw e;
...
// DELETE /edxx/fab/maintenance/plans/:id — 删除保养计划（不影响设备）
...
// POST /edxx/fab/operations/cleanup — 按 id 数组清理报工记录 { ids: number[] }
...
// POST /edxx/fab/operations/cleanup-by-equipment — 清理指定设备的全部报工引用 { equipment_id: number }
