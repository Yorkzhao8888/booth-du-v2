// BOOTH-STATION-04: 清理 ST01 回归冒烟站 id=16/17/18 (P2 数据清理)
// 幂等可重跑; 先备份后删除; 保护站 id=1/2/15 不动
// 用法: NODE_PATH=node_modules node -r dotenv/config scripts/station04-data-fix.cjs
const fs = require('fs');
const path = require('path');
const { Pool } = require('pg');
const pool = new Pool({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } });
const TARGET_IDS = [16, 17, 18];
const PROTECTED_IDS = [1, 2, 15]; // 沿用 station02 模式: 产线1/2号 + ST01 验收保留站
...
(async () => {
  ...
  if (nonTestJobs.length) {
    console.error('ABORT: 三站存在非 ST01-* 前缀的工单引用, 需人工裁定:', JSON.stringify(nonTestJobs.map((j) => ({ id: j.id, job_id: j.job_id }))));
    process.exit(1);
  }

  // ---- 1. 备份 (站点完整行含 metadata/state_history + 关联工单完整行) ----
  const backupPath = path.join(__dirname, 'station04-backup.json');
